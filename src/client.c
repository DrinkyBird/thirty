		...
		if (client->mapsend_state == mapsend_success) {
			for (int i = 0; i < 4; i++) {
				if (client->mapgz_buffer == NULL || buffer_size(client->mapgz_buffer) == buffer_tell(client->mapgz_buffer)) {
					...
					client_send_env_colour(client, env_colour_sky);
					client_send_env_colour(client, env_colour_cloud);
					client_send_env_colour(client, env_colour_fog);
					client_send_env_colour(client, env_colour_ambient);
					client_send_env_colour(client, env_colour_sunlight);
					client_send_env_colour(client, env_colour_skybox);
...
void client_disconnect(client_t *client, const char *msg) {
	...
	if (client->spawned) {
		...
		for (size_t i = 0; i < server.num_clients; i++) {
			...
			buffer_write_uint8(other->out_buffer, other->protocol_version == 1 ? packet_player_despawn_c0015 : packet_player_despawn);
			buffer_write_int8(other->out_buffer, client->idx);
			client_flush(other);
		}
	}
...
void client_send_env_colour(client_t *client, envcolourtype_t colour) {
	rgb_t *rgb = &server.map->colours[colour];
